...
#    sys.stdout = open(log_file_name, 'w', 1)
    ...
    if not gfile.Exists(data_file_name):
        data_file_name = os.path.join(FLAGS.data_dir, "part-{0:0>5}".format(0))
...
#                    _, step_loss, step_count = sess.run([model.train_op, model.loss_fn, model.global_step], options=options)
                    if local_step == 3:
