...
# Kit: For distributed training
tf.app.flags.DEFINE_string(
    "ps_hosts", "localhost:8700", "Comma-separated list of hostname:port pairs.")

tf.app.flags.DEFINE_string(
    "worker_hosts", "localhost:8800", "Comma-separated list of hostname:port pairs.")
...
def main(_):
  ...
  if FLAGS.job_name == "ps":
    ...
    with tf.Graph().as_default():
      ...
      acc_time = 0.0
      acc_loss = 0.0
      while current_step < FLAGS.max_number_of_steps:
        current_step += 1
        start_time = time.time()
        step_loss = sess.run(train_tensor)
        acc_time += time.time() - start_time
        acc_loss += step_loss
        duration += time.time() - start_time
        if current_step % FLAGS.log_every_n_steps == 0:
            acc_time /= FLAGS.log_every_n_steps
            acc_loss /= FLAGS.log_every_n_steps
            print ("Step %d: time = %f seconds, step_time = %f seconds, loss = %f" %(current_step, duration, acc_time, acc_loss))
            acc_time = 0.0
            acc_loss = 0.0

  else:
    print("Unknown job name [%s]." %(FLAGS.job_name))
