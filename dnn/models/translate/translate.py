...
import models.translate.data_utils as data_utils
import models.translate.seq2seq_model as seq2seq_model
...
def create_model(FLAGS, session, forward_only):
...
def train():
  ...
  with tf.Session() as sess:
    ...
    model = create_model(FLAGS, sess, False)
...
def dist_train(FLAGS_, server, cluster):
  import tempfile
  ...
  with tf.device(tf.train.replica_device_setter(
    ...
    # Create model.
    print("Creating %d layers of %d units." % (FLAGS.num_layers, FLAGS.size))
    model = create_model(FLAGS, None, False)
  ...
  previous_losses = []
  duration = 0
  print ("Kit: Start working. batch_size = %d, epoch = %d" % (FLAGS.batch_size, FLAGS.epoch))
