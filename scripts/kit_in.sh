...
data_path="/home/${username}/run/dnn/data/imagenet"
    ...
    while IFS= read -r line; do
        ...
        then
            ...
            then
                ...
                server_cmd+="echo \"python -u models/slim/dist_train_image_classifier.py --ps_hosts=${ps_list} --worker_hosts=${worker_list} --job_name=ps --task_index=${index} 2>&1 | tee s.result.txt \" >> ${run_path}/s.sh"
            fi
    ...
    while IFS= read -r line; do
        ...
        then
            ...
            then
                ...
                worker_cmd+="echo \"python -u models/slim/dist_train_image_classifier.py --dataset_dir=${data_path}  --model_name=${network} --batch_size=4 --optimizer=adam --learning_rate=0.001 --ps_hosts=${ps_list} --worker_hosts=${worker_list} --job_name=worker --task_index=${index} $* 2>&1 | tee w.result.txt  \" >> ${run_path}/w.sh"
            fi
