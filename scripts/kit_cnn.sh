    ...
    while IFS= read -r line; do
        ...
        then
            ...
            then
                ...
                server_cmd+="echo \"CUDA_VISIABLE_DEVICE="" python -u kit_cnn.py --ps_hosts=${ps_list} --worker_hosts=${worker_list} --job_name=ps --task_index=${index} 2>&1 | tee s.result.txt \" >> ${run_path}/s.sh"
            fi
    ...
    while IFS= read -r line; do
        ...
        then
            ...
            then
                ...
                worker_cmd+="echo \"CUDA_VISIABLE_DEVICE="" python -u kit_cnn.py --network=${network} --ps_hosts=${ps_list} --worker_hosts=${worker_list} --job_name=worker --task_index=${index} $* 2>&1 | tee w.result.txt  \" >> ${run_path}/w.sh"
            fi
